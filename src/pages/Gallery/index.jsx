// import gimage1 from "../../images/gimage1-min.webp";
import gimage2 from "../../images/gimage2-min.webp";
import gimage3 from "../../images/gimage3-min.webp";
import gimage4 from "../../images/gimage4-min.webp";
import gimage5 from "../../images/gimage5-min.webp";
import gimage6 from "../../images/gimage6-min.webp";
import gimage7 from "../../images/gimage7-min.webp";
import gimage8 from "../../images/gimage8-min.webp";
import gimage9 from "../../images/gimage9-min.webp";
// import gimage10 from "../../images/gimage10-min.webp";
import gimage11 from "../../images/gimage11-min.webp";
import gimage12 from "../../images/gimage12-min.webp";
import gimage13 from "../../images/gimage13-min.webp";
import gimage14 from "../../images/gimage14-min.webp";
import gimage15 from "../../images/gimage15-min.webp";
import gimage16 from "../../images/gimage16-min.webp";
import gimage17 from "../../images/gimage17-min.webp";
import gimage18 from "../../images/gimage18-min.webp";
import gimage19 from "../../images/gimage19-min.webp";
import gimage20 from "../../images/gimage20-min.webp";
import gimage21 from "../../images/gimage21-min.webp";
import gimage22 from "../../images/gimage22-min.webp";
import gimage23 from "../../images/gimage23-min.webp";
import gimage24 from "../../images/gimage24-min.webp";
import gimage25 from "../../images/gimage25-min.webp";
import gimage26 from "../../images/gimage26-min.webp";
import gimage27 from "../../images/gimage27-min.webp";
import gimage28 from "../../images/gimage28-min.webp";
import gimage29 from "../../images/gimage29-min.webp";
import gimage30 from "../../images/gimage30-min.webp";
import gimage31 from "../../images/gimage31-min.webp";
import gimage32 from "../../images/gimage32-min.webp";
import gimage33 from "../../images/gimage33-min.webp";
import gimage34 from "../../images/gimage34-min.webp";
import gimage35 from "../../images/gimage35-min.webp";
// import gimage36 from "../../images/gimage36-min.webp";
import gimage37 from "../../images/gimage37-min.webp";
import gimage38 from "../../images/gimage38-min.webp";
import CountDownThumbnail from "../../images/CountDownThumbnail.webp";
const Gallery = () => {
	return (
		<div className="bg-[#0B0A1B] py-10">
			<div className="flex flex-col md:flex-row items-center justify-center px-5 md:px-10 w-full">
				{/* First Column Images */}
				<div className="flex flex-col">
					<div className="h-96 flex flex-col">
						{" "}
						<h3 className="text-white text-6xl font-bold">A NIGHT</h3>
						<h3 className="text-white text-6xl font-bold">TO</h3>
						<h3 className="text-white text-6xl font-bold">REMEMBER</h3>
						<h3 className="text-[#3F2EA1] text-6xl font-bold">YOUR</h3>
						<h3 className="text-[#3F2EA1] text-6xl font-bold">ENTIRE LIFE</h3>
					</div>
					<img className="h-96 w-auto mt-10" src={gimage4} alt="galleryImage" />
				</div>

				{/* Second Column Images */}
				<div className="flex flex-col mt-8 md:mt-0 md:ml-10">
					<img className="h-96 w-auto" src={gimage2} alt="galleryImage" />
					<img className="h-96 w-full mt-10" src={gimage3} alt="galleryImage" />
				</div>
			</div>

			<div className="flex flex-col w-full justify-center items-center">
				<div className="max-w-[1269px]">
					{/* Second Row */}
					<div className="px-5 md:px-10 py-10 ">
						<img
							className="h-auto w-full"
							src={CountDownThumbnail}
							alt="galleryImage"
						/>
					</div>

					{/* Third Row */}
					<h2 className="px-10 pb-10 text-[#D44865] font-bold text-5xl md:text-7xl text-center">
						DO JOIN US FOR THE GRAND EVENT
					</h2>

					<div className="flex flex-col md:flex-row w-full items-center">
						<img
							className="py-5 px-5 md:px-10 md:w-1/3 h-30"
							src={gimage5}
							alt="galleryImage"
						/>
						<img
							className="py-5 px-5 md:px-10 md:w-1/3 h-30"
							src={gimage6}
							alt="galleryImage"
						/>
						<img
							className="py-5 px-5 md:px-10 md:w-1/3 h-30"
							src={gimage7}
							alt="galleryImage"
						/>
					</div>

					<img
						className="w-full h-auto object-contain px-5 md:px-10 mt-10"
						src={gimage8}
						alt="galleryImage"
					/>

					<img
						className="w-full h-auto object-contain px-5 md:px-10 mt-10"
						src={gimage9}
						alt="galleryImage"
					/>
					<img
						className="w-full h-auto object-contain px-5 md:px-10 mt-10"
						src={gimage23}
						alt="galleryImage"
					/>

					<div className="flex flex-col md:flex-row w-full items-center">
						<img
							className="py-5 px-5 md:px-10 md:w-1/3 h-30"
							src={gimage11}
							alt="galleryImage"
						/>
						<img
							className="py-5 px-5 md:px-10 md:w-1/3 h-30"
							src={gimage12}
							alt="galleryImage"
						/>
						<img
							className="py-5 px-5 md:px-10 md:w-1/3 h-30"
							src={gimage13}
							alt="galleryImage"
						/>
					</div>
					<img
						className="w-full h-auto object-contain px-5 md:px-10 mt-10"
						src={gimage37}
						alt="galleryImage"
					/>

					
					<div className="flex flex-col md:flex-row w-full items-center">
						<img
							className="py-5 px-5 md:px-10 md:w-1/3 h-30"
							src={gimage16}
							alt="galleryImage"
						/>
						<img
							className="py-5 px-5 md:px-10 md:w-1/3 h-30"
							src={gimage17}
							alt="galleryImage"
						/>
						<img
							className="py-5 px-5 md:px-10 md:w-1/3 h-30"
							src={gimage18}
							alt="galleryImage"
						/>
					</div>
					
					

					
					<img
						className="w-full h-auto object-contain px-5 md:px-10 mt-10"
						src={gimage25}
						alt="galleryImage"
					/>
					<img
						className="w-full h-auto object-contain px-5 md:px-10 mt-10"
						src={gimage26}
						alt="galleryImage"
					/>
					<div className="flex flex-col md:flex-row w-full items-center">
						<img
							className="py-5 px-5 md:px-10 md:w-1/3 h-30"
							src={gimage27}
							alt="galleryImage"
						/>
						<img
							className="py-5 px-5 md:px-10 md:w-1/3 h-30"
							src={gimage28}
							alt="galleryImage"
						/>
						<img
							className="py-5 px-5 md:px-10 md:w-1/3 h-30"
							src={gimage29}
							alt="galleryImage"
						/>
					</div>
					<img
						className="w-full h-auto object-contain px-5 md:px-10 mt-10"
						src={gimage30}
						alt="galleryImage"
					/>
					<div className="flex flex-col md:flex-row w-full items-center">
						<img
							className="py-5 px-5 md:px-10 md:w-1/3 h-30"
							src={gimage31}
							alt="galleryImage"
						/>
						<img
							className="py-5 px-5 md:px-10 md:w-1/3 h-30"
							src={gimage32}
							alt="galleryImage"
						/>
						<img
							className="py-5 px-5 md:px-10 md:w-1/3 h-30"
							src={gimage20}
							alt="galleryImage"
						/>
					</div>
					<img
						className="w-full h-auto object-contain px-5 md:px-10 mt-10"
						src={gimage38}
						alt="galleryImage"
					/>
					<div className="flex flex-col md:flex-row w-full items-center">
						<img
							className="py-5 px-5 md:px-10 md:w-1/3 h-30"
							src={gimage35}
							alt="galleryImage"
						/>
						<img
							className="py-5 px-5 md:px-10 md:w-1/3 h-30"
							src={gimage34}
							alt="galleryImage"
						/>
						<img
							className="py-5 px-5 md:px-10 md:w-1/3 h-30"
							src={gimage33}
							alt="galleryImage"
						/>
					</div>
					
				</div>
			</div>
		</div>
	);
};
export default Gallery;
