...
const EventRegisterForm = (props) => {
	...
	const validatePhone = (data) => {
		...
		!res && toast.error("Please Enter a Valid PhoneNumber!");
		return res;
	};
	...
	const validateSap = (data) => {
		...
		!res && toast.error("Please Enter a Valid SAP/ADHAAR!");
		return res;
	};
	...
	const validateName = (data) => {
		var res =
			...
			/^[a-zA-Z]+$/.test(data.participant_name) &&
			data.participant_name.length > 2 &&
			data.participant_name.length < 13;
	...
	return (
		<>
			...
			{isTeam ? (
				isDit ? (
					<Registerteamdit handleChange={handleChange} submit={submit} />
				) : (
					<Registerteam handleChange={handleChange} submit={submit} />
				)
			) : isDit ? (
				<RegisterIndvdit handleChange={handleChange} submit={submit} />
			) : (
				<RegisterIndv handleChange={handleChange} submit={submit} />
			)}
		</>
	);
};
